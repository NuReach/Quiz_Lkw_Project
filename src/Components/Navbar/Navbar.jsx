  ...
  return (
    <nav className='bg-black flex justify-between items-center px-6 shadow-2xl '>
        ...
        <div className='relative'>
          <input placeholder='Input Quiz Code...' className=' bg-inherit lg:w-96 text-xs rounded-md text-white border-2 border-white focus:border-2 focus:border-white focus:ring-0  transition'  type="text" value={quizId} onChange={(e)=>setQuizId(e.target.value)} />
          <button onClick={handleClick} className='text-white text-xs absolute  z-10 right-0 rounded-r-md bg-green-500 h-full border-2 border-l-0 p-1 border-white '>GO</button>
        </div>
        <div className='flex space-x-6 items-center'>
          <button onClick={()=>setdailog(!dailog)} className='py-2 px-4 bg-white dark:text-white dark:bg-pink-600 rounded-md text-sm font-medium hover:scale-105 transition'>Create</button>
