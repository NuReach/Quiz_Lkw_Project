...
export default function OPplaying({item,number,quizId}) {
  ...
  return (
    ...
    <div className='flex flex-wrap justify-center items-center  '>
          <div onClick={(e)=>handleClick(e,"op1")} className={answer.find((item)=>item== "op1" ) ?'bg-red-600 border-8 border-green-300 cursor-pointer m-1 text-white text-sm p-3 flex items-center space-x-3 w-96 h-24 font-medium rounded-sm':'bg-red-600 cursor-pointer m-1 text-white text-sm p-3 flex items-center space-x-3 w-96 h-24 font-medium rounded-sm'}>
              <div className='p-4'>
              <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"><path fill="currentColor" d="M1 21h22L12 2"/></svg>
              </div>
              <p> {item.quiz.anw1}</p>
          </div>
          <div onClick={(e)=>handleClick(e,"op2")} className={answer.find((item)=>item== "op2" ) ?'bg-blue-600 m-1 border-8 border-green-300 cursor-pointer text-white text-sm p-3 flex space-x-3 items-center w-96 h-24 font-medium rounded-sm' :'bg-blue-600 m-1 cursor-pointer text-white text-sm p-3 flex space-x-3 items-center w-96 h-24 font-medium rounded-sm'}>
            <div className='p-4'>
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M3 21V3h18v18z"/></svg>
            </div>
            <p>{item.quiz.anw2}</p>
          </div>
          <div onClick={(e)=>handleClick(e,"op3")} className={ answer.find((item)=>item== "op3" ) ?'bg-yellow-400 cursor-pointer border-8 border-green-300 m-1 text-white text-sm p-3 flex items-center space-x-3 w-96 h-24 font-medium rounded-sm': 'bg-yellow-400 cursor-pointer m-1 text-white text-sm p-3 flex items-center space-x-3 w-96 h-24 font-medium rounded-sm'}>
              <div className='p-4'>
          ...
          <div onClick={(e)=>handleClick(e,"op4")} className={answer.find((item)=>item== "op4" ) ? 'bg-green-500 m-1 cursor-pointer border-8 border-green-300 text-white text-sm p-3 flex space-x-3 items-center w-96 h-24 font-medium rounded-sm' : 'bg-green-500 m-1 cursor-pointer text-white text-sm p-3 flex space-x-3 items-center w-96 h-24 font-medium rounded-sm'}>
